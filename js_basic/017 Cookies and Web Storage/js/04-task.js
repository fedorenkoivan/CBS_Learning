/*
            Завдання:
            Зробіть так, щоб зміни у масиві users зберігалися і були доступні
            навіть після закриття і повторного відкриття браузера.
        */
        let users = [
            {
                balance: '1250.89',
        ...
        class ListService {
            ...
            // видалення даних з розмітки та масиву
            deleteSelectedItem() {
                const index = this.data.indexOf(this.selectedData);
                if (index != -1) {
        ...
        // Клас для роботи з формою, редактує інфо про користувача
        ...
        if(formStorage) users = JSON.parse(formStorage);
